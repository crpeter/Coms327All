...
static char get_char_for_object(object *o) {
  char temp;
...
static void color_thing(uint32_t num, uint32_t type) {
  if (type == 1) {
    switch (num) {
      case 1:
      ...
      if ((illuminated = is_illuminated(d->PC, y, x))) {
        ...
        if (d->character_map[y][x] &&
          ...
          if (d->character_map[y][x] != d->PC) {
            int at = d->character_map[y][x]->color;
            color_thing(at, 1);
          }
          ...
          visible_monsters++;
                 ...
                 can_see(d,
                         character_get_pos(d->PC),
                         d->object_map[y][x]->position,
                         1, 0)) {
          color_thing(d->object_map[y][x]->color, 1);
          mvaddch(y + 1, x,
                  get_char_for_object(d->object_map[y][x]));
          color_thing(d->object_map[y][x]->color, 0);
      } else {
    ...
    for (x = 0; x < 80; x++) {
      if (d->character_map[y][x]) {
        ...
        if (d->character_map[y][x] != d->PC) {
          ...
          color_thing(d->object_map[y][x]->color, 1);
          mvaddch(y + 1, x, get_char_for_object(d->object_map[y][x]));
          color_thing(d->object_map[y][x]->color, 0);
      } else {
        switch (mapxy(x, y)) {
        case ter_wall:
