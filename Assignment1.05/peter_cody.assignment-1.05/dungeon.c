...
int gen_dungeon(dungeon_t *d)
{
    ...
    // place a up or down stair
    int progress_stairs = 0;

    while (progress_stairs != 2) {
        int x = rand_range(1, DUNGEON_X);
        int y = rand_range(1, DUNGEON_Y);
        if (mapxy(x, y) == ter_floor_room) {
            if (progress_stairs == 0) {
                mapxy(x, y) = stair_up;
                progress_stairs++;
            } else {
                mapxy(x, y) = stair_down;
                progress_stairs++;
            }
        }
    }
